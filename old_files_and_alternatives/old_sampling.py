...
def compute_fft_order_from_distance_matrix(
    ...
    """
    Compute FFT order of cities given precomputed distance matrix.
    d_mat: n_cities x n_cities distance matrix, in order of city_ids
    city_ids: list of community ids
    reps: dict community -> representative vertex index
    deg: degree array for all vertices
    seed: for shuffling
    """
    rng = np.random.default_rng(seed)
    shuffled_city_ids = city_ids.copy()
    rng.shuffle(shuffled_city_ids)

    n_cities = len(city_ids)
    index_map = {cid: i for i, cid in enumerate(city_ids)}
    shuffled_indices = [index_map[cid] for cid in shuffled_city_ids]
    d_mat_shuffled = d_mat[np.ix_(shuffled_indices, shuffled_indices)]
    d_mat_shuffled = _replace_infinite(d_mat_shuffled)

    # Choose seed: rep with highest degree
    rep_degrees = [deg[reps[cid]] for cid in shuffled_city_ids]
    best_idx = int(np.argmax(rep_degrees))

    order = [shuffled_city_ids[best_idx]]
    chosen_indices = {best_idx}
    dist_to_centers = d_mat_shuffled[best_idx].copy()
    dist_to_centers[best_idx] = -1.0
    ...
    while len(order) < n_cities:
        ...
        if next_idx in chosen_indices:
            ...
            if not remaining:
                break
            next_idx = remaining[0]
        order.append(shuffled_city_ids[next_idx])
        chosen_indices.add(next_idx)
        new_dists = d_mat_shuffled[next_idx]
        dist_to_centers = np.minimum(dist_to_centers, new_dists)
        dist_to_centers[next_idx] = -1.0
